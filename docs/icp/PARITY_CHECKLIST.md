...
| OP-016 | ZenDB dependency | Proposed authoritative and archive store; source/dependency/Candid/Wasm baseline is pinned at released `v2.0.1` in `third_party/zendb/v2.0.1.pin.json` with the local-only reproducer and findings in `docs/icp/ZENDB_PIN_AND_BASELINE.md`. `M1BoundedBenchmark.mo` and `test-zendb-benchmark.sh` add an unexecuted synthetic expected/2x bounded-data harness that reports only candidate-exposed query/replace/delete instructions and document/index bytes; the runner fails closed on an unhealthy local DFX replica, names `--network local`/`--no-wallet`, and skips Mops's ICP API-compatibility probe. Insert/reindex instruction, archive-byte, low-cycle, logical-ID recovery, RBAC-revocation, and mutation-recovery proofs remain required. Approve a collection-specific fallback where proof fails; never assume an unmerged/future PR | IN_PROGRESS / BLOCKED_M1/G2 |
