...
| Didit | Direct-provider session and signed HTTP-update webhook ingress | The workflow is ID verification, liveness, face match, AML, and proof of address; phone verification, IP analysis, database validation, and full-report retrieval are prohibited. Before session creation, caller declares document issuing country; it is retained only in the one-use seven-day correlation, must select an enabled Foundation country policy, and must exactly match Didit's minimum attestation or KYC/payout activation fails closed. Pin the exact Didit API/workflow/webhook signature contract. Accept only the bounded attestation allowlist, deduplicate events, retain AML rejection precedence, and fail closed if Didit cannot supply the minimum result. Didit-side blocklists may enforce anti-evasion; the canister never receives their document/biometric material. |
