...
- `application/private collections`: versioned identity/role/profile/KYC/hold, canonical AI result/source, redacted audit, stable schedule/cursor, completion-receipt, immutable application-to-treasury outbox tuples, encrypted-evidence, migration, and archive collections. New AI execution has no task/DAG/lease/provider-batch collections; fixed bounded `llm` operations execute directly in versioned Motoko code.
- `treasury/private collections`: append-only accounting, payment obligations/operations/attempts, immutable application-to-treasury inbox tuples, and replay/finality collections with the unified treasury's Motoko operation/reconciliation protocol.
...
| `email`, `emailVerified` | Private verified notification email only; it is never login authority, public data, or marketing contact, and is deleted on account closure subject to a concrete legal hold. Legacy source conflicts are reported; raw verification values are never imported. |
| `votingPleaUnsubscribed` | `User.notificationPreferences.votingPleaUnsubscribed` |
| `name`, `onboarded`, `createdAt`, `updatedAt` | Preserve only approved non-PII state/timestamps. Legal name is not imported into target live state or public views; leaderboard uses a shortened principal and no username exists. Source-name preservation is a redacted migration exception. See `G2_PRIVACY_AND_GOVERNANCE_DECISION.md`. |
...
| `orcidId`, `githubHandle`, `bitbucketHandle`, `gitlabHandle` | External-account records. A private immutable provider subject is required to mark a handle `ownershipVerified`; an imported handle without that proof is `legacyUnverified`. The certified ban-voting projection may expose only provider, public handle/ORCID ID, and that verification state; it exposes neither the subject nor any wallet/payout field. |
...
| `kycStatus`, `kycVerifiedAt`, `kycRejectedAt`, `kycRejectionReason`, `kycData` | No raw KYC value is imported or stored. Target `evidence_kyc_v1` is the minimum principal-bound attestation: `approved`/`declined`/`review`, policy/workflow version, issuing country, time, expiry, immutable internal audit-event ID, and only if necessary a bounded closed rejection code. Full report, provider report/session ID, raw callback, document/media, DOB, address, ID/personal number, biometric, and provider response are excluded from canister state, export, and backup. A separate caller-bound opaque callback correlation lasts at most seven days and is not an imported attestation. |
...
| `BatchMapping` | `id`, `customId`, `batchId`, `createdAt` → restricted legacy provider-item metadata; response is represented only by a bounded redacted canonical result/hash in `archive/ZenDB ai_artifact_v1` | Historical-only `customId` uniqueness and relation preserved; no live provider-item state is created. A sanitized historical payload is allowed only for a specifically documented migration-reconciliation exception with a redaction profile and deadline. |
...
| `NonBatchMapping` | `id`, `customId`, `nonBatchId`, `createdAt` → restricted legacy provider-item metadata plus only a bounded redacted canonical result/hash | Historical-only invariants match the source; no live provider-item state is created. A sanitized historical payload requires its own documented migration-reconciliation exception, redaction profile, and deadline. |
...
| `OpenAILog` | `id,userId,taskId,customId,storeId,runnerClassName,requestInitiated,responseReceived,errorMessage,createdAt,updatedAt` → ZenDB log metadata; `requestData,responseData` → only a bounded redacted canonical result/hash in `archive/ZenDB ai_artifact_v1` | Exact owner/evaluation-operation indexes; no textual JSON owner search; imported task reference is historical only; redaction classification; missing/nulled historical responses represented explicitly. A sanitized historical request/response payload needs a specifically documented migration-reconciliation exception, redaction profile, and deadline. |
...
`ai_result_migration_exceptions(customId, sourceTable, sourceRowId, resultKind, responseData, reason, createdAt)` is exported explicitly, but its target evidence disposition is limited to migration/table/source-row identifiers, bounded reason/conflict codes, timestamp, canonical-row hash, and optional related target logical-ID/content-hash. `responseData` and every other raw row field are not imported. A separately documented migration-reconciliation exception may retain a sanitized payload in `ai_artifact_v1` only with its redaction profile and deadline. Primary key `(customId,sourceTable)` is preserved in the source reconciliation report. It participates in count/hash reports even though Prisma cannot query it.
...
Required inventory per physical table: row and index bytes/counts; min/max IDs/timestamps; max/percentile text/JSON sizes; null/distinct counts; normalized identity/address duplicates; FK orphans; status histograms; sequence state; exact decimal scale/range/sums; extra Global rows; AI-source conflicts; financial cross-table reconciliation; primary/replica identity and update/delete key; direct-CDC versus redacted-projection membership; and publication/output-plugin compatibility. The unrun M1 aggregate-only command is documented in `POSTGRES_INVENTORY.md`; it supplies the safe baseline evidence but does not replace the G2-approved explicit source projections or cutover rehearsal. Secret, bearer, and raw-token values are reported only by approved source identifier/fingerprint/digest.

Before a sensitive table is imported, the G2 record additionally names the purpose/data controller/legal basis, field minimization, retention/deletion and backup/restore disposition, access-audit role, financial or anti-evasion exception, and whether any derived public field is allowed. Raw KYC PII and sensitive identity attributes are excluded rather than encrypted for target import. Missing policy is a blocking exception, not a reason to import unrestricted evidence.
