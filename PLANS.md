...
- **M1 decision recorded 2026-08-07:** ZenDB runs only as an in-process library inside a persistent storage-authority canister; it is not a separately reachable remote ZenDB canister and is not the RBAC authority. That canister is the sole Candid boundary, authenticates a fixed application-canister/governance allowlist, authorizes the exact collection/action and logical-ID scope, and exposes no generic ZenDB or grant API. Domain canisters retain resource authorization as a separate layer. The detailed handoff and required proof are in `docs/icp/M1_OPERATOR_HANDOFF.md`.
- M1 is blocked until the self-hosted authoritative-storage design proves a mutation/recovery protocol for every multi-record and cross-canister invariant. A call from a domain canister to the storage authority may lose its reply; the caller journals before that call, the storage operation is idempotent by immutable logical ID/hash, and recovery leaves no partially activated authority. If that proof fails for a collection, G2 must record a narrowly scoped native-Motoko exception and its rationale rather than silently falling back for all authority.
