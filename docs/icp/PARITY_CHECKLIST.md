...
| DM-004 | All PKs, uniques, indexes, relations, delete rules | Versioned Motoko records and the explicit ZenDB collection/index/grant/limit catalogue are in `M1_STORAGE_SCHEMA_AND_SAGAS.md`. The 2026-08-07 local-only `storage_authority` scaffold adds a fixed six-principal Motoko RBAC matrix, fixed owner probe surface, governance-only audit, no generic ZenDB/grant API, and deterministic unit negatives (`StorageAuthorityPolicy.test.mo`). `test-storage-authority-boundary.sh` passes against an isolated disposable local DFX replica: anonymous direct ingress and bootstrap/deployer direct access are denied; actual core, unrelated, and governance canister callers prove owner-only access plus cross-owner, malformed-ID, governance-as-data, and governance-audit behavior. It upgrades the same storage-authority artifact and repeats those cases, proving that the empty actor's fixed caller matrix survives a local upgrade; the runner isolates its DFX version probe and replica launch from ambient identities. It persists no data and remains neither a ZenDB nor future-state-upgrade/recovery proof. The pinned candidate's synthetic PocketIC remote-RBAC baseline proves ungranted and collection-scoped-writer negatives plus collection isolation (`test-zendb-rbac.sh`/`v2.0.1.rbac-proof.json`). The target-data runner retains local proof for logical-ID recovery, owner lost-reply/duplicate reconciliation, archive non-activation, and bounded v1→v2 repair. Its owner-controlled exact-artifact upgrade proof failed on 2026-08-07: ZenDB `v2.0.1` restored the revoked bootstrap admin grant after upgrade (`test-zendb-authoritative.sh`/`v2.0.1.authoritative-proof.json`). It is therefore ineligible as authoritative storage; G2 needs a named collection-specific native-Motoko exception or a later pin must pass the proof. | IN_PROGRESS / BLOCKED_M1/G2 |
