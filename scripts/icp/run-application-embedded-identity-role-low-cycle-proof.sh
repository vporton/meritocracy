...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/m1-application-identity-role-low-cycle.XXXXXX")"
trap 'rm -rf -- "$runtime_dir"' EXIT
cp -- "$pocket_ic_bin" "$runtime_dir/pocket-ic"
chmod 700 "$runtime_dir/pocket-ic"
node "$repo_root/scripts/icp/application-embedded-identity-role-low-cycle-pocketic-proof.cjs" "$runtime_dir/pocket-ic" "$artifact_dir/application_embedded_identity_role_proof.wasm"
echo "Consolidated-application embedded identity/role low-cycle PocketIC proof passed."
