...
import Store "../canisters/treasury/EmbeddedApplicationOperationInboxStore";
...
let input : Saga.Input = {
  logicalId = "application-treasury:v1:inbox-17";
  version = 1;
  contentHash = hash(9);
};
...
assert Store.decideIdempotentWrite(input, ?{ input with logicalId = "application-treasury:v1:inbox-18" }) == #conflict;
