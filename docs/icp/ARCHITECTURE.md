...
KYC and identity evidence cannot be activated merely because encryption exists. Before collection/import, G2 records its purpose, data controller/legal basis, minimization, retention and cryptographic-erasure schedule, backup/restore behavior, access-audit roles, and financial/anti-evasion retention exceptions. The owner-approved G2 policy input permits certified public projections to expose only verified display legal name, caller principal, payout addresses, payment history, and evaluation history, and only through a field allowlist for the specific projection. Raw social identities remain excluded, and the permission does not make every field in any underlying record public. Published treasury receiving addresses remain separately governed asset/scope configuration.
...
7. Public views have an explicit field allowlist. The G2 policy input permits only verified display legal name, caller principal, payout addresses, payment history, and evaluation history as public identity/activity classes; each projection still declares its exact fields and purpose. Raw social identifiers remain excluded unless a later narrowly recorded decision, following applicable-law review, permits a specified field and purpose.
...
| KYC raw evidence | Encrypted, access-logged private application collection | G2-approved purpose/legal basis, minimization, retention/cryptographic erasure, backup, access audit, and accounting/anti-evasion exception policy; minimize indexed sensitive fields |
