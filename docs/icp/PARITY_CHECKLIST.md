...
**M1 version-bound identity/role boundary (2026-09-11):** The two fixed synthetic embedded adapter schemas now persist an immutable desired remote version with the logical ID and 32-byte content hash. The passing pure vectors reject a changed version or hash; the PocketIC fixture runner is updated to repeat those checks after its EOP upgrade, but that linked execution must be rerun before it is new proof evidence. This is adapter/fixture evidence only: it adds no storage-authority Candid method, ZenDB grant, target data, deployment, caller authorization, durable core intent, or lost-reply proof.

**M1 fixed-core lost-reply runner (2026-09-11):** The disposable core/authority fixtures now have a SHA-256-verified build and an isolated PocketIC runner. It uses only synthetic principals/canisters, rejects outsider direct authority and non-operator core ingress, forces a post-`await` lost reply, upgrades the core with EOP before recovery, and accepts recovery only on the fixed core-only logical-ID/version/hash lookup. Each Wasm upload is bounded to 1,000,000 bytes; the runner uses a disposable byte-identical copy of the pinned PocketIC binary and never invokes DFX or accepts a wallet/identity. Syntax, pure vectors, and relevant canister checks pass, but the linked fixture build exceeded the local interactive ceiling and no execution pass is claimed. CI retains the reproducible build/proof logs. No project canister Candid surface, target data, deployment, or authority was enabled.
