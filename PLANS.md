...
20. **INCOMPLETE 2026-09-10 (real embedded storage-authority upgrade proof):** The staged proof implementation is complete, but its required PocketIC execution is incomplete. `scripts/icp/build-storage-authority-upgrade-proof.sh` builds only the pre-embedded authority, current embedded authority, and fixed caller fixture WASMs; `scripts/icp/run-storage-authority-upgrade-proof.sh` then runs only the PocketIC proof, accepting a cached pinned binary through `POCKET_IC_BIN`. The proof installs the exact pre-embedded commit `4b9e51377ca351b4a162c5d102b258536d539ba2`, establishes and checks the complete fixed caller matrix, upgrades with PocketIC EOP `install_code` to the current embedded-store WASM, verifies the original matrix and all direct/inter-canister denials remain enforced, then performs a second real upgrade of the embedded artifact and repeats preservation/denial checks. Any replaced matrix, reset/bypassed authorization, or failed upgrade is an explicit failure. `.github/workflows/m1-storage-authority-upgrade.yml` runs the isolated build and proof with a 20-minute job limit, caches the pinned PocketIC toolchain, and retains the generated artifact hashes/logs. **Execution in the Codex workspace remains incomplete: the isolated artifact build passed and SHA-256 verification passed; the pinned PocketIC 12.0.0 binary reports its expected version, but the local server did not become ready before the client startup timeout, even when invoked from a temporary writable copy through `POCKET_IC_BIN`. No PocketIC proof success has been recorded. M1 remains IN_PROGRESS and NOT VERIFIED until this exact external/CI proof succeeds.** This does not prove collection persistence, bounded operations, lost-reply/duplicate recovery, archive, low-cycle, or repair/resume behavior.
