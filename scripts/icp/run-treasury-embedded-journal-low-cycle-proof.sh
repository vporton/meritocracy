#!/usr/bin/env bash
# Runs only the checksum-verified disposable low-cycle treasury-journal fixture.
set -euo pipefail
...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/m1-treasury-journal-low-cycle.XXXXXX")"
trap 'rm -rf -- "$runtime_dir"' EXIT
cp -- "$pocket_ic_bin" "$runtime_dir/pocket-ic"
chmod 700 "$runtime_dir/pocket-ic"
node "$repo_root/scripts/icp/treasury-embedded-journal-low-cycle-pocketic-proof.cjs" "$runtime_dir/pocket-ic" "$artifact_dir/treasury_embedded_journal_proof.wasm"
echo "Treasury embedded journal low-cycle PocketIC proof passed."
