#!/usr/bin/env bash
# Runs only checksum-verified synthetic canonical archive-export capacity proof.
set -euo pipefail
...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/m1-treasury-journal-archive-export-capacity.XXXXXX")"
report_path="$runtime_dir/report.json"
trap 'rm -rf -- "$runtime_dir"' EXIT
cp -- "$pocket_ic_bin" "$runtime_dir/pocket-ic"
chmod 700 "$runtime_dir/pocket-ic"
node "$repo_root/scripts/icp/treasury-journal-archive-export-capacity-pocketic-proof.cjs" "$runtime_dir/pocket-ic" "$artifact_dir/treasury_journal_archive_export_fixture.wasm" "$artifact_dir/treasury_journal_archive_export_sink.wasm" "$report_path"
install -m 600 "$report_path" "$artifact_dir/capacity-report.json"
echo "Treasury-journal canonical archive-export capacity proof passed."
