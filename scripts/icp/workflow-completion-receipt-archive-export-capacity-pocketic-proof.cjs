#!/usr/bin/env node
// Synthetic-only bounded archive capacity proof. No target API, workflow
// result, provider payload, archive service, or production input is used.
...
function batch(name, count) { return Array.from({ length: count }, (_, ordinal) => ({ logicalId: `workflow-receipt:v1:archive-capacity:${name}:${ordinal}`, cycleId: `cycle:archive-capacity:${name}:${ordinal}`, operationName: "archive-proof", desiredVersion: 1n, contentHash: hash(ordinal) })); }
