...
This contract does not satisfy M1 acceptance by itself. The remaining suite must prove these exact records/catalogue against the pinned ZenDB API: collection-scope RBAC/direct-ingress negatives, bootstrap revocation and post-upgrade audit; unique logical-ID conflict handling; bounded cursor/index plans; interruption before/during/after remote writes and manifest activation; duplicate delivery; upgrades; low cycles; archive failure; and collection-vN repair/resume. A collection that fails its proof needs a G2-approved, named native-Motoko exception in `SCHEMA_MAPPING.md`; it cannot silently share a general fallback.
