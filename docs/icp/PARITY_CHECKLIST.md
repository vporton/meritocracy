...
**M1 embedded storage-state boundary (2026-09-10):** The fixed-policy `storage_authority` now initializes the exact pinned in-process ZenDB `VersionedStableStore` as private persistent state, bound to its own principal. Its regenerated stable baseline records that type before deployment. No Candid method, collection, document, index, generic CRUD/filter, remote ZenDB actor, ZenDB grant API, target data, deployment, or authority was added; the existing policy probes remain the entire public surface. This is compilation/stable-shape evidence only, and cannot satisfy the required bounded mutation, recovery, archive, capacity, repair, or upgrade proofs.
