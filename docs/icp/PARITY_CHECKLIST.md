...
**M1 pre-await interruption runner extension (2026-09-11):** The same synthetic core now persists an identity-binding intent and deliberately traps before its authority `await`. Following a core EOP upgrade, the fixed lookup must return `absent` and the only available retry takes no input, so it can resend only the durable logical ID/version/hash tuple before reconciliation of its deliberately lost reply. Pure intent vectors and the project core stable check pass. The linked fixture build exceeds this workspace's interactive command ceiling, so this extension has no local PocketIC pass claim until the amended isolated CI job completes. It adds no project Candid surface, target data, deployment, or storage-authority claim.
