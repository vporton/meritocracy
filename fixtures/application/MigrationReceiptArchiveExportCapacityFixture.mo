...
// batch before its first await and is neither an importer nor an archive API.
...
shared ({ caller = installer }) persistent actor class (operator : Principal, sinkId : Principal) = this {
  ...
  public shared ({ caller }) func retainBatch(inputs : [Receipt.Input]) : async [Archive.ArchiveTuple] {
    onlyOperator(caller);
    let ?bindings = prepareBatch(inputs) else throw Error.reject("invalid synthetic migration archive capacity batch");
    var receipts : [Archive.ArchiveTuple] = [];
    for (index in bindings.keys()) {
      let binding = bindings[index];
      switch (Embedded.write(receiptStore, inputs[index])) {
        case (#acknowledged) {};
        case (#blocked) { throw Error.reject("invalid synthetic migration archive capacity receipt") };
        case (#conflict) { throw Error.reject("synthetic migration archive capacity receipt conflict") };
        case (#storageError) { throw Error.reject("synthetic migration archive capacity receipt storage error") };
      };
      let receipt = await sink.retain(binding);
      if (receipt != binding.tuple) throw Error.reject("synthetic migration archive capacity receipt changed");
