...
26. **IMPLEMENTED 2026-09-11 (fixed core lost-reply execution proof):** Added the isolated build/run pair `scripts/icp/build-core-identity-role-lost-reply-proof.sh` and `scripts/icp/run-core-identity-role-lost-reply-proof.sh`, plus its PocketIC bridge and 20-minute GitHub Actions job. The runner builds and SHA-256-verifies only the two disposable synthetic fixtures, creates synthetic authority/core canisters, supplies their known canister principals as class arguments, denies outsider direct authority write/lookup and non-operator core ingress, journals before the core's authority `await`, requires the deliberately lost reply to reject, performs an EOP upgrade of the core before recovery, and requires the configured core-only exact lookup to reconcile as `acknowledge`. Chunks are at most 1,000,000 bytes; the runner copies the pinned PocketIC binary byte-for-byte into a disposable writable `/tmp` path and uses no DFX, identity, wallet, external network, target data, deployment, or project-canister Candid method. Shell/JavaScript syntax checks, `mops test IdentityRoleIntent`, `mops check core_canister`, and `mops check storage_authority` pass. On 2026-09-11, the SHA-256 verification and complete PocketIC proof passed in an isolated local environment: the initial core authority call rejected after the remote write, the core EOP upgrade retained its pre-await durable intent, and recovery acknowledged only after the authority's fixed core-only lookup observed the exact logical-ID/version/hash. This is not an authoritative collection claim: duplicate delivery, authority upgrade, interruption phases, archive, low-cycle, repair/resume, and all non-identity collections remain M1/G2 blockers.
