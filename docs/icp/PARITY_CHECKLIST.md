...
| OP-016 | ZenDB dependency | Proposed authoritative and archive store; source/dependency/Candid/Wasm baseline is pinned at released `v2.0.1` in `third_party/zendb/v2.0.1.pin.json` with the local-only reproducer and findings in `docs/icp/ZENDB_PIN_AND_BASELINE.md`; still prove logical-ID/hash reconciliation, least-privilege collection RBAC and bootstrap revocation; run export/reindex/collection-vN/mutation-recovery tests; approve collection-specific fallback where proof fails; never assume an unmerged/future PR | IN_PROGRESS / BLOCKED_M1/G2 |
