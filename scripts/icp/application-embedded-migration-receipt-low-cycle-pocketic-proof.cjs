...
// private migration-receipt collection. It has no importer or application API.
...
const management = Principal.fromText("aaaaa-aa"),
  lowInstallationCycles = 100_000_000_000n;
const idl = ({ IDL: C }) =>
  C.Service({
    retainReceiptThenWrite: C.Func([Input], [Result], []),
    retainReceiptForOperatorRepair: C.Func([Input], [Result], []),
    retryRetainedReceipt: C.Func([], [Result], []),
    repairRetainedReceipt: C.Func([], [Result], []),
  });
...
async function main() {
  ...
  try {
    const installer = Principal.fromUint8Array(Uint8Array.of(4, 3));
    const id = await pic.createCanister({
      sender: installer,
      controllers: [installer],
      cycles: lowInstallationCycles,
    });
    await install(pic, installer, id);
    const actor = pic.createActor(idl, id);
    actor.setPrincipal(installer);
    const input = {
      logicalId: "migration-receipt:v1:low-cycles",
      migrationId: "low-cycle-migration",
      sourceTable: "User",
      chunk: 0n,
      rowCount: 1,
      payloadHash: hash(7),
    ...
    expect(
      await actor.retainReceiptThenWrite(input),
      "blocked",
      "low-cycle receipt is retained but not written",
    );
    expect(
      await actor.retainReceiptThenWrite({ ...input, payloadHash: hash(8) }),
      "blocked",
      "retained receipt rejects a replacement",
    );
    expect(
      await actor.retryRetainedReceipt(),
      "blocked",
      "low-cycle retry has no private write",
    );
    ...
    expect(
      await actor.retryRetainedReceipt(),
      "blocked",
      "upgraded low-cycle retry retains intent but has no private write",
    );
    if ((await pic.addCycles(id, 2_000_000_000_000n)) < 1_000_000_000_000n)
      throw new Error(
        "low-cycle proof failed to replenish disposable migration fixture",
      );
    expect(
      await actor.retryRetainedReceipt(),
      "acknowledged",
      "replenished retry writes only retained receipt",
    );
    expect(
      await actor.retryRetainedReceipt(),
      "blocked",
    ...
    const interrupted = {
      ...input,
      logicalId: "migration-receipt:v1:operator-repair",
      chunk: 1n,
      payloadHash: hash(9),
      contentHash: hash(79),
    };
    expect(
      await actor.retainReceiptForOperatorRepair(interrupted),
      "blocked",
      "interrupted receipt retains no automatic write path",
    );
    expect(
      await actor.retryRetainedReceipt(),
      "blocked",
      "ordinary retry cannot bypass operator repair",
    );
    actor.setPrincipal(Principal.fromUint8Array(Uint8Array.of(4, 9)));
    expect(
      await actor.repairRetainedReceipt(),
      "blocked",
      "outsider cannot repair retained receipt",
    );
    actor.setPrincipal(installer);
    await install(pic, installer, id, {
      upgrade: [{ skip_pre_upgrade: [], wasm_memory_persistence: [{ keep: null }] }],
    ...
    expect(
      await actor.repairRetainedReceipt(),
      "acknowledged",
      "authorized no-input repair writes retained receipt after upgrade",
    );
    expect(
      await actor.repairRetainedReceipt(),
      "blocked",
      "consumed repaired receipt cannot replay",
    );
    console.log(
      "application embedded migration receipt low-cycle proof passed",
    );
  } finally {
    await pic.tearDown();
    await server.stop();
