...
| DM-004 | All PKs, uniques, indexes, relations, delete rules | Versioned Motoko records and the explicit ZenDB collection/index/grant/limit catalogue are in `M1_STORAGE_SCHEMA_AND_SAGAS.md`. The pinned candidate's synthetic PocketIC remote-RBAC baseline proves ungranted and collection-scoped-writer negatives plus collection isolation (`test-zendb-rbac.sh`/`v2.0.1.rbac-proof.json`). The target-data proof also passes on the pinned DFX local replica: unique logical-ID rejection, one-result hash recovery, and opaque one-document cursor advancement are executed against the remote candidate (`test-zendb-authoritative.sh`/`v2.0.1.authoritative-proof.json`). Its cursor is an internal document-ID repair cursor, not application logical-ID ordering; the Mops single-ingress runner remains unsuitable for the linked test actor. Motoko enforcement, bootstrap/post-upgrade grant audit, collection-vN rebuild, and mutation-recovery tests remain required | IN_PROGRESS / BLOCKED_M1/G2 |
...
| OP-016 | ZenDB dependency | Proposed authoritative and archive store; source/dependency/Candid/Wasm baseline is pinned at released `v2.0.1` in `third_party/zendb/v2.0.1.pin.json` with the local-only reproducer and findings in `docs/icp/ZENDB_PIN_AND_BASELINE.md`. The synthetic expected/2x bounded-data harness (`M1BoundedBenchmark.mo`/`test-zendb-benchmark.sh`) passed on 2026-08-02 with candidate-exposed query/replace/delete instructions and document/index bytes recorded in `v2.0.1.benchmark-proof.json`; it also proves owner-side document/batch over-limit rejection before a remote call. The separate target-data harness (`M1AuthoritativeProof.mo`/`test-zendb-authoritative.sh`) passed on the same local-only boundary, proving unique logical-ID rejection, one-result hash recovery, and opaque cursor advancement; its record is `v2.0.1.authoritative-proof.json`. The runners use fresh ephemeral project-local loopback DFX replicas, fail closed on an unhealthy replica, and name `--network local`/`--no-wallet` without Mops's ICP API-compatibility probe. Insert/reindex instruction, archive-byte, low-cycle, RBAC-revocation, and mutation-recovery proofs remain required. Approve a collection-specific fallback where proof fails; never assume an unmerged/future PR | IN_PROGRESS / BLOCKED_M1/G2 |
