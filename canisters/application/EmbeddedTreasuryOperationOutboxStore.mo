...
import Saga "TreasuryOperationSaga";
import StorageCatalog "../shared/StorageCatalog";

/// Application-private durable outbox for the only application-to-treasury
/// boundary. It stores exactly the immutable recovery tuple, never a payment
/// operation, amount, asset, destination, caller, or chain material.
module {
  ...
  public func create(store : ZenDB.Types.VersionedStableStore) : ?Store {
    ...
    let #ok(collection) = db.createCollection<Record>(
      "application_treasury_outbox_v1", schema, candify,
  ...
  public func reopen(store : ZenDB.Types.VersionedStableStore) : ?Store {
    let db = ZenDB.launchDefaultDB(store);
    let #ok(collection) = db.getCollection<Record>("application_treasury_outbox_v1", candify) else return null;
    ?collection;
  };

  public func validEncoding(input : Saga.Input) : Bool {
    // The contract bounds the text to 512 scalar values and fixes the hash at
    // 32 bytes. Its Candid envelope is safely below the catalogue document cap.
    Saga.prepare(input) != null and 4_096 <= StorageCatalog.limits.maxDocumentBytes;
  };

  /// A duplicate can be acknowledged only when the immutable recovery tuple
  /// remains byte-for-byte equivalent. An absent observation is not success:
  /// the caller must use the retained intent's bounded write/recovery route.
