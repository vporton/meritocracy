...
**M1 compatibility-fork boundary (2026-09-09):** Following owner approval, `identify` resolves to a local audited `0.0.2-m1.1` fork. It excludes only the upstream frontend E2E actor fixtures and changes its 103 backend `mo:core` imports to explicit `mo:core@1`; the public API entry is byte-identical and `mops test OAuthAttempt` passes its caller/state/expiry/replay/unsupported-provider vectors. `verify-identify-m1-compat.sh` fixes the source and package hashes and rejects any wider delta. The owner subsequently directed a narrow local `hex-m1-compat@1.0.2-m1.1` replacement under the unchanged `mo:hex` import: it covers only the stateless formatting API used by identify, is hash-pinned by `verify-hex-m1-compat.sh`, and its compact/upper/URL/empty/verbose vectors pass. No provider, credential, OAuth callback, Candid surface, stable signature, target authorization, storage method, data, or deployment changed. The full check now advances past hex and fails independently in ZenDB's `memory-collection@0.4.0` at `Nat.bytes` under the unqualified Core-1 binding; do not patch it without a new scoped disposition.

**M1 Mops package conversion (2026-08-09):** `third_party/` was removed. ZenDB `v2.0.1` remains a direct Mops dependency with its full released content closure locked in `mops.lock`; its retained proof records live in `docs/icp/evidence/zendb/`. The local identify and hex compatibility packages are pinned by verifiers because Mops local paths have no lockfile file-hash section. That earlier remote-CanisterDB Git pin is a distinct historical artifact, not validation of the Mops package. No target canister imports ZenDB, no storage method or data was enabled, and the next `memory-collection@0.4.0` compiler failure remains an M1 integration blocker.
