#!/usr/bin/env node
// M1 synthetic capacity boundary for the real fixed migration-receipt adapter.
// It has no PostgreSQL client, source data, credential, target importer, DFX,
// identity, wallet, or external-network access.
...
const [bin, authorityWasm, importerWasm, reportPath] = process.argv.slice(2);
if (!bin || !authorityWasm || !importerWasm || !reportPath) throw new Error("Expected PocketIC binary, authority Wasm, importer Wasm, and report path");
...
const importerIdl = ({ IDL: C }) => C.Service({ writeThenLoseReply: C.Func([Input], [], []), reconcileLostReply: C.Func([], [Recovery], []) });
...
async function startPocketIc(binPath) {
  const runtimeDir = fs.mkdtempSync(path.join(os.tmpdir(), "m1-migration-capacity-pocketic-server-")), portFile = path.join(runtimeDir, "port");
...
async function scenario(pic, installer, operator, name, count) {
  ...
  const importerId = await pic.createCanister({ sender: installer, controllers: [installer], cycles: 20_000_000_000_000n });
  const config = { core: Principal.fromUint8Array(Uint8Array.of(7, 1)), workflow: Principal.fromUint8Array(Uint8Array.of(7, 2)), treasury: Principal.fromUint8Array(Uint8Array.of(7, 3)), archive: importerId, evidence: Principal.fromUint8Array(Uint8Array.of(7, 4)), governance: Principal.fromUint8Array(Uint8Array.of(7, 5)) };
  await install(pic, installer, authorityId, authorityWasm, IDL.encode([Config], [config]));
  await install(pic, installer, importerId, importerWasm, IDL.encode([P, P], [operator, authorityId]));
  const before = { authority: await pic.getCyclesBalance(authorityId), importer: await pic.getCyclesBalance(importerId) }, importer = pic.createActor(importerIdl, importerId); importer.setPrincipal(operator); let encodedInputBytes = 0;
  const invalid = { logicalId: "x".repeat(513), migrationId: "capacity-invalid", sourceTable: "User", chunk: 0n, rowCount: 1, payloadHash: hash(254), desiredVersion: 1n, contentHash: hash(255) };
  await reject(() => importer.writeThenLoseReply(invalid), `${name} over-limit input is rejected before storage`);
  expect(await importer.reconcileLostReply(), "blocked", `${name} over-limit input creates no journal`);
  for (let ordinal = 0; ordinal < count; ordinal += 1) { const input = { logicalId: `migration-receipt:v1:capacity:${name}:${ordinal}`, migrationId: `capacity-migration:${name}`, sourceTable: "User", chunk: BigInt(ordinal), rowCount: ordinal + 1, payloadHash: hash(ordinal), desiredVersion: 1n, contentHash: hash(ordinal + count) }; encodedInputBytes += IDL.encode([Input], [input]).byteLength; await reject(() => importer.writeThenLoseReply(input), `${name} write ${ordinal} deliberately loses reply`); expect(await importer.reconcileLostReply(), "acknowledge", `${name} write ${ordinal} exact tuple reconciles`); }
  const after = { authority: await pic.getCyclesBalance(authorityId), importer: await pic.getCyclesBalance(importerId) };
  return { name, writes: count, lookups: count * 2, encodedInputBytes, authorityCycleDelta: (before.authority - after.authority).toString(), importerCycleDelta: (before.importer - after.importer).toString() };
}
async function main() {
  const server = await startPocketIc(bin), pic = await PocketIc.create(server.url);
  try { const installer = Principal.fromUint8Array(Uint8Array.of(7, 31)), operator = Principal.fromUint8Array(Uint8Array.of(7, 32)); const report = { schemaVersion: 1, component: "M1 fixed migration-receipt adapter capacity proof", emulator: "PocketIC synthetic-only", scenarios: [await scenario(pic, installer, operator, "expected", 16), await scenario(pic, installer, operator, "two_x", 32)], rejection: { logicalIdBytes: 513, result: "rejected before journal or storage in each scenario" }, limitations: ["Cycle deltas are emulator measurements, not a mainnet instruction or production-cycle budget.", "This covers only the fixed migration-receipt adapter; other collection-specific capacity proofs remain required before G2."] }; fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600, flag: "wx" }); console.log(JSON.stringify(report)); } finally { await pic.tearDown(); await server.stop(); }
}
