...
16. **IN_PROGRESS 2026-08-09 (Mops package conversion):** Removed `third_party/`; `mops install --lock update` now resolves released `identify@0.0.2` and `zendb@2.0.1` directly, with their full file-hash closure committed in `mops.lock`. Retained ZenDB pin/RBAC/authoritative/benchmark records moved to `docs/icp/evidence/zendb/`, and the verifier follows that location. They describe a distinct historical remote-CanisterDB Git candidate, not the Mops package. This adds no ZenDB import to a target canister, storage method, target data, authority, deployment, or compatibility fork. The direct package `mops check` is currently blocked before application compilation because the released identify package contains `src/frontend/agent-js/e2e/node/canisters/counter.mo`, an actor Motoko 1.4 rejects in a library search path. An owner-approved compatible identify release or minimal audited fork remains required before the combined closure can be accepted; do not silently restore the vendored view or alter package source.
