...
**M1 fixed-core lost-reply fixture (2026-09-11):** `IdentityRoleIntent.mo` and its passing vectors fix the durable core transition: validate and journal the immutable input before the only storage-authority `await`, retain it if the reply is lost, and reconcile only through an exact bounded logical-ID/version/hash observation. The paired disposable core/authority fixtures type-check; the authority accepts only its configured synthetic core canister and returns no sensitive payload from the fixed lookup. PocketIC execution of the deliberate post-`await` trap, recovery, ingress denial, and upgrade retention remains required before this becomes lost-reply proof. No project canister Candid surface, target data, deployment, or authority was enabled.
