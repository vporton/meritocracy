...
// migration-receipt collection. It has no importer or application Candid API.
...
async function scenario(pic, installer, name, count) {
  ...
  const tooLong = { logicalId: "x".repeat(513), migrationId: "capacity-rejection", sourceTable: "User", chunk: 0n, rowCount: 1, payloadHash: hash(250), desiredVersion: 1n, contentHash: hash(249) };
  expect(await actor.writeReceipt(tooLong), "blocked", `${name} rejects over-limit logical ID`);
  let encodedInputBytes = 0;
  for (let n = 0; n < count; n += 1) {
    const receipt = { logicalId: `migration-receipt:v1:capacity:${name}:User:${n}`, migrationId: `capacity-migration:${name}`, sourceTable: "User", chunk: BigInt(n), rowCount: 1, payloadHash: hash(n), desiredVersion: 1n, contentHash: hash(n + 64) };
...
async function main() {
  ...
  try {
    const installer = Principal.fromUint8Array(Uint8Array.of(4, 2));
    const scenarios = [await scenario(pic, installer, "expected", 16), await scenario(pic, installer, "two_x", 32)];
    const report = { schemaVersion: 1, component: "M1 consolidated application embedded migration receipt capacity proof", emulator: "PocketIC synthetic-only", scenarios, rejection: { logicalIdBytes: 513, result: "rejected before a private collection insert" }, limitations: ["This measures bounded synthetic collection ingress only; it is not a production instruction or cycle budget.", "No importer, PostgreSQL connection, source row, application public method, caller authorization policy, target data, or deployment is involved."] };
