...
**M1 compatibility-fork boundary (2026-09-09):** Following owner approval, `identify` resolves to a local audited `0.0.2-m1.1` fork. It excludes only the upstream frontend E2E actor fixtures and changes its 103 backend `mo:core` imports to explicit `mo:core@1`; the public API entry is byte-identical and `mops test OAuthAttempt` passes its caller/state/expiry/replay/unsupported-provider vectors. `verify-identify-m1-compat.sh` fixes the source and package hashes and rejects any wider delta. No provider, credential, OAuth callback, Candid surface, stable signature, target authorization, storage method, data, or deployment changed. The original actor packaging failure is resolved, but combined `mops check` now reaches the separate `hex@1.0.2` Motoko-1.4 `Text.join` incompatibility. Do not widen the identify fork or patch `hex` without a separately approved and equivalently audited disposition.

**M1 Mops package conversion (2026-08-09):** `third_party/` was removed. ZenDB `v2.0.1` remains a direct Mops dependency with its full released content closure locked in `mops.lock`; its retained proof records live in `docs/icp/evidence/zendb/`. The separately approved local identify compatibility fork is pinned by its verifier because Mops local paths have no lockfile file-hash section. That earlier remote-CanisterDB Git pin is a distinct historical artifact, not validation of the Mops package. No target canister imports ZenDB, no storage method or data was enabled, and the subsequent `hex@1.0.2` compiler failure remains an M1 integration blocker.
...
| OP-016 | ZenDB dependency | ZenDB `v2.0.1` source/dependency/Candid/Wasm baseline remains pinned, but it is not an authoritative or archive-store candidate: the 2026-08-07 owner-controlled exact-artifact upgrade proof showed that it restores a revoked bootstrap admin grant. The Motoko 1.4.1 repository compiler accepts the isolated exact-source probe. The approved identify fork pins its Core 1 imports explicitly, but the combined Mops closure presently fails at distinct `hex@1.0.2` `Text.join` incompatibility; no dual compiler or unreviewed hex fork is approved. The bounded benchmark and other local evidence remain diagnostic only. A G2-approved named native-Motoko exception is required per affected collection, or a later exact pin must pass the complete proof suite; do not assume an unmerged/future PR. | IN_PROGRESS / BLOCKED_M1/G2 |
