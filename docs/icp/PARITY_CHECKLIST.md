...
| DM-005 | 17 explicit Prisma transaction call sites | M1 now specifies the native intent/version/hash/manifest recovery state machine. The pure `MutationRecovery.mo` vectors prove exact desired-version/hash acknowledgement, identical-only retry from an absent or unchanged complete CAS target, fail-closed partial/malformed/concurrent cases, and bounded pending-only manifest activation. `IdentityRoleRecovery.mo` additionally rejects anonymous/malformed identity and role inputs, incomplete OAuth evidence, and non-exact duplicate logical IDs; its passing new vectors reject changed versions as well as hashes. The isolated `EmbeddedIdentityRoleStoreProof.mo` fixture and PocketIC runner retain synthetic-only initial acknowledgement, exact retry, malformed-ID, anonymous-principal rejection, and EOP retention checks; their newly added version-conflict assertions need a fresh linked fixture execution before they count as evidence. The 2026-09-11 local run passed in the isolated PocketIC-capable environment; the workspace sandbox's bridge-start deadline is an environment limitation, while CI retains the reproducible build/proof logs. These contracts and fixtures persist no target data and are not a storage-authority mutation/recovery proof; domain-caller journaling, Motoko authorization, and fault injection at every phase remain required | IN_PROGRESS / BLOCKED_M1/G2 |
