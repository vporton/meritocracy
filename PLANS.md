...
24. **IN_PROGRESS 2026-09-11 (synthetic identity/role upgrade-retention proof):** The isolated PocketIC runner now performs an EOP `install_chunked_code` upgrade using only the same SHA-256-verified synthetic fixture Wasm after the initial fixed binding/role writes and negative checks. It then requires each exact retry to acknowledge and each changed-hash retry to conflict, which distinguishes retained immutable records from an empty or replaced collection. The builder's published checksum now names the artifact relatively rather than its removed disposable staging path, so the runner validates the published pair before execution. Shell/JavaScript syntax checks and `mops test IdentityRoleRecovery` pass; the linked fixture build passes and its checksum verifies. On 2026-09-11 the local PocketIC bridge did not become ready before its own bounded startup deadline, so this modified upgrade proof has no local pass claim. CI must execute and retain the result before it can count as interruption/upgrade evidence. It remains synthetic-only and does not expose a storage-authority Candid method, authorize a domain caller, establish lost-reply recovery, or make either collection authoritative.
