...
1. Add pinned ICP/Motoko/Mops toolchain manifests and empty canister interfaces. Pin the exact Motoko `indentify` package version/source/package/API hashes and record a provider-capability matrix (PKCE/public-client or confidential-client flow, configured redirect/client binding, immutable-subject source, issuer/audience availability, required scopes, token-retention need, and retirement outcome). Compile only a no-live-provider caller-bound-attempt interface/vector fixture; it must contain no provider secret or credential.
2. Add a read-only PostgreSQL inventory command with a hard read-only transaction and safe output, including logical-decoding/version/replica-identity/WAL-capacity capability checks and the redaction-projection inventory; it must not create a production publication, slot, trigger, or outbox.
