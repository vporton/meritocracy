...
26. **IN_PROGRESS 2026-09-11 (fixed core lost-reply fixture):** Added the pure `canisters/core/IdentityRoleIntent.mo` transition contract and vectors, plus two disposable synthetic fixtures for the fixed core-to-storage-authority principal-binding path. The core fixture persists the validated immutable binding input and `remoteWriteStarted` phase before its only authority `await`; after an acknowledged fixed write it deliberately traps, leaving the pre-await journal for a later fixed logical-ID lookup. Reconciliation uses only the retained logical ID/version/hash and acknowledges only the exact returned tuple; absent state permits an identical retry and a changed version/hash conflicts. The authority fixture accepts only its configured synthetic core canister and exposes only that fixed binding write/lookup pair, whose lookup is bounded to two documents and returns no identity payload. `mops test IdentityRoleIntent`, `mops check core_canister`, `mops check storage_authority`, and direct Motoko type checks of both fixtures pass. This is not yet execution evidence: add the isolated PocketIC runner that proves the post-`await` trap, recovery call, direct-ingress negative, and upgrade retention before claiming an actual lost-reply proof. No production Candid method, target data, deployment, caller-configurable collection/action, ZenDB grant, legacy behavior, production signing authority, or asset movement changed.
