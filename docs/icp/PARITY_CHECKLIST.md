...
Last updated: 2026-08-02. The repository's AGPL-3.0-only license and related package metadata changes are complete and recorded in `9122ff0`; no additional relicensing gate remains. M1 now includes an unrun, aggregate-only PostgreSQL inventory command (`docs/icp/POSTGRES_INVENTORY.md`); it records no target-feature or production-inventory completion evidence. This remains the exhaustive legacy audit baseline.
...
| DM-004 | All PKs, uniques, indexes, relations, delete rules | Versioned Motoko records and the explicit ZenDB collection/index/grant/limit catalogue are in `M1_STORAGE_SCHEMA_AND_SAGAS.md`. The pinned candidate's synthetic PocketIC remote-RBAC baseline proves ungranted and collection-scoped-writer negatives plus collection isolation (`test-zendb-rbac.sh`/`v2.0.1.rbac-proof.json`). The target-data runner retains its executed local proof for unique logical-ID rejection, one-result hash recovery, opaque cursor advancement, owner lost-reply/duplicate reconciliation, archive failure non-activation, bootstrap/self-grant revocation, and bounded v1→v2 page replay/repair (`test-zendb-authoritative.sh`/`v2.0.1.authoritative-proof.json`). Its external anonymous upgrade attempt was correctly rejected because the child database remains controlled by the synthetic owner, so post-upgrade grant-revocation is still unexecuted. Its cursor is an internal document-ID repair cursor, not application logical-ID ordering; the Mops single-ingress runner remains unsuitable for the linked test actor. Motoko enforcement, owner-controlled post-upgrade proof, and mutation-recovery tests remain required | IN_PROGRESS / BLOCKED_M1/G2 |
| DM-005 | 17 explicit Prisma transaction call sites | M1 now specifies the native intent/hash/manifest recovery state machine; Motoko authorization and fault injection at every phase remain required | IN_PROGRESS / BLOCKED_M1/G2 |
...
| OP-016 | ZenDB dependency | Proposed authoritative and archive store; source/dependency/Candid/Wasm baseline is pinned at released `v2.0.1` in `third_party/zendb/v2.0.1.pin.json` with the local-only reproducer and findings in `docs/icp/ZENDB_PIN_AND_BASELINE.md`. The synthetic expected/2x bounded-data harness (`M1BoundedBenchmark.mo`/`test-zendb-benchmark.sh`) passed on 2026-08-02 with candidate-exposed query/replace/delete instructions and document/index bytes recorded in `v2.0.1.benchmark-proof.json`; it also proves owner-side document/batch over-limit rejection before a remote call. The separate target-data harness (`M1AuthoritativeProof.mo`/`test-zendb-authoritative.sh`) retains its executed local proof for unique logical-ID rejection, one-result hash recovery, opaque cursor advancement, owner lost-reply, archive failure non-activation, bootstrap/self-grant revocation, and bounded v1→v2 replay/resume. The external anonymous upgrade attempt is correctly rejected because the synthetic owner controls its database child; no post-upgrade grant assertion is claimed until an owner-controlled upgrade path is proven. Its exact boundary is in `v2.0.1.authoritative-proof.json`. The runners use fresh ephemeral project-local loopback replicas, fail closed on an unhealthy replica, and name `--network local`/`--no-wallet` without Mops's ICP API-compatibility probe. Insert/reindex instruction, archive-byte, low-cycle, executed post-upgrade revocation, and complete mutation-recovery proofs remain required. Approve a collection-specific fallback where proof fails; never assume an unmerged/future PR | IN_PROGRESS / BLOCKED_M1/G2 |
