...
| DM-004 | All PKs, uniques, indexes, relations, delete rules | Versioned Motoko records and the explicit ZenDB collection/index/grant/limit catalogue are in `M1_STORAGE_SCHEMA_AND_SAGAS.md`. The pinned candidate's synthetic PocketIC remote-RBAC baseline proves ungranted and collection-scoped-writer negatives plus collection isolation (`test-zendb-rbac.sh`/`v2.0.1.rbac-proof.json`). The target-data runner retains its executed local proof for unique logical-ID rejection, one-result hash recovery, and opaque cursor advancement; it now also implements owner lost-reply/duplicate reconciliation, bootstrap/self-grant and post-upgrade revocation assertions, plus a bounded v1→v2 page-replay repair fixture (`test-zendb-authoritative.sh`/`v2.0.1.authoritative-proof.json`). The extension has no execution evidence: its no-login local rerun reached the build but produced no Wasm, deployment, or test call. Its cursor is an internal document-ID repair cursor, not application logical-ID ordering; the Mops single-ingress runner remains unsuitable for the linked test actor. Motoko enforcement, executed collection-vN rebuild, and mutation-recovery tests remain required | IN_PROGRESS / BLOCKED_M1/G2 |
