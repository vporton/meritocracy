...
for artifact in treasury_journal_authority.wasm treasury_journal_fixture.wasm SHA256SUMS; do [[ -f "$artifact_dir/$artifact" ]] || { echo "Missing treasury-journal proof artifact: $artifact" >&2; exit 1; }; done
...
node "$repo_root/scripts/icp/treasury-journal-lost-reply-pocketic-proof.cjs" "$runtime_dir/pocket-ic" "$artifact_dir/treasury_journal_authority.wasm" "$artifact_dir/treasury_journal_fixture.wasm"
