...
9. **IN_PROGRESS 2026-08-07 (persistent policy state; DFX proof correction):** The storage authority now stores its fixed six-principal matrix in a private persistent field, and the generated stable signature records it. An upgrade cannot replace that matrix merely by receiving a new actor-class init argument, and no public configuration mutation exists. `mops test`, `mops check`, `mops build`, and `mops check-stable` pass. **Correction to item 8:** its DFX runner was not valid evidence and is now disabled; DFX 0.32 identities are global, and a local run observed an ambient home-directory identity despite the script's intended configuration isolation. No production network, canister, data, or asset was touched, but M1's cryptographic authority-boundary rule means the runner must not be used. The required replacement is an identity-free, pinned PocketIC proof using synthetic principals; it must cover direct ingress, inter-canister owner/cross-owner/governance negatives, bootstrap/deployer denial, and an upgrade supplied a different valid init matrix that cannot replace the persisted matrix. This remains non-authoritative scaffolding: ZenDB persistence, target data, mutation/recovery, archive, low-cycle, repair/resume, and the PocketIC proof are outstanding.
