...
- Raw KYC documents, DOB, address, photo, document/ID number, biometrics, provider reports, and sensitive identity attributes never enter staging, target data, canonical export, CI artifacts, or operator backups. The only target KYC disposition is the G2-approved minimum attestation; source fields excluded by that policy require explicit redacted migration exceptions. There is no country eligibility allowlist: private issuing-country data is limited to AML/audit context and cannot itself enable or deny KYC or payouts. Activation instead requires the normal identity-proof checks and a current, positive AML/sanctions decision; a negative, ambiguous, missing, or stale result fails closed and leaves the liability held. Certified public projections may expose only their allowlisted principal, payout address, payment/evaluation history, and, in ban-voting only, provider/public handle-or-ORCID-ID plus verification state. Published treasury receiving configuration is separate.
- The migration does not authorize continued legacy custody: no raw wallet key is generated for migration, and unknown legacy sends remain manual-reconciliation exceptions. Production cutover requires the legacy sender inventory and pause/approved-expiring-exception evidence defined in `PLANS.md`.
...
5. votes, holds, KYC/liveliness evidence, and notification state, each with its approved evidence-lifecycle/public-visibility classification;
...
2. Announce maintenance; stop legacy write/API workers and every cron/payment sender. Preserve read-only service where safe. Before the freeze, verify the legacy-sender inventory: each real-value sender is paused or has a separately approved, expiring exception with named operator, asset/scope, compensating controls, and rollback action; no unknown send is reset/retried automatically.
