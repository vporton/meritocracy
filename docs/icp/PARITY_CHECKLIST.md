...
| DM-005 | 17 explicit Prisma transaction call sites | M1 now specifies the native intent/version/hash/manifest recovery state machine. The pure `MutationRecovery.mo` vectors prove exact desired-version/hash acknowledgement, identical-only retry from an absent or unchanged complete CAS target, fail-closed partial/malformed/concurrent cases, and bounded pending-only manifest activation. `IdentityRoleRecovery.mo` additionally rejects anonymous/malformed identity and role inputs, incomplete OAuth evidence, and non-exact duplicate logical IDs; its passing new vectors reject changed versions as well as hashes. The isolated `EmbeddedIdentityRoleStoreProof.mo` fixture and PocketIC runner retain synthetic-only initial acknowledgement, exact retry, malformed-ID, anonymous-principal rejection, and EOP retention checks. The fixed synthetic core-to-authority proof additionally journals before its authority `await`, deliberately loses the reply after the remote write, EOP-upgrades the core, and acknowledges recovery only after its fixed core-only authority lookup observes the exact logical ID/version/hash; outsider authority calls and non-operator core ingress are denied. The 2026-09-11 isolated local PocketIC executions passed; the workspace sandbox's bridge-start deadline is an environment limitation, while CI retains reproducible build/proof logs. These contracts and fixtures persist no target data and are not a storage-authority mutation/recovery proof; duplicate delivery, authority upgrade, fault injection at every phase, and all remaining collection recovery proofs are required | IN_PROGRESS / BLOCKED_M1/G2 |
