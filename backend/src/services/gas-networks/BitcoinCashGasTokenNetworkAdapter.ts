...
const readBitcoinCashConfig = (): BitcoinNetworkConfig => ({
  enabled: (process.env.ENABLE_BCH === 'true') || (process.env.BCH_ENABLED === 'true'),
  networkId: 'bitcoin-cash-mainnet',
  networkName: 'Bitcoin Cash Mainnet',
  nativeSymbol: 'BCH',
  nativeDecimals: 8,
  walletAddress: process.env.BCH_WALLET_ADDRESS ?? process.env.BITCOIN_WALLET_ADDRESS,
  rpcUrl: process.env.BCH_RPC_URL ?? process.env.BITCOIN_RPC_URL,
  rpcUsername: process.env.BCH_RPC_USERNAME ?? process.env.BITCOIN_RPC_USERNAME,
  rpcPassword: process.env.BCH_RPC_PASSWORD ?? process.env.BITCOIN_RPC_PASSWORD,
  headers: process.env.BCH_RPC_KEY
    ? { Authorization: `Bearer ${process.env.BCH_RPC_KEY}` }
    : process.env.BITCOIN_RPC_KEY
...
export class BitcoinCashGasTokenNetworkAdapter implements GasTokenNetworkAdapter {
  ...
  private ensureStaticWalletAddress(config: BitcoinNetworkConfig): string | undefined {
    ...
    if (config.wif) {
      try {
        ...
        console.warn(`⚠️  [Bitcoin Cash] Failed to derive address from WIF: ${message}`);
      }
    }
  ...
  private async getFeeRate(config: BitcoinNetworkConfig, client: RpcCommandClient): Promise<number> {
    try {
      ...
      console.warn(`⚠️ [Bitcoin Cash] Failed to estimate fee via RPC: ${error}`);
    }
  ...
  private async tryWalletlessBalanceLookup(
    ...
    if (!address) {
      ...
      console.warn(
        'ℹ️  [Bitcoin Cash] Unable to determine wallet address; set BCH_WALLET_ADDRESS (or BCH_WIF locally) to enable balance queries without wallet RPC support.'
      );
    ...
    if (!this.fallbackBalanceNoticeShown) {
      console.log(
        'ℹ️  [Bitcoin Cash] Wallet RPC is unavailable; falling back to `scantxoutset` balance queries for address-based monitoring.'
      );
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`⚠️  [Bitcoin Cash] UTXO scan balance fallback failed: ${message}`);
      return undefined;
    }
  }
  ...
  async getNetworkContexts(tokenOptions: TokenDistributionOptions): Promise<GasTokenNetworkContext[]> {
    ...
    if (!config.rpcUrl/* || !config.rpcUsername || !config.rpcPassword*/) {
      console.warn('⚠️  [Bitcoin Cash] Missing RPC configuration, skipping.');
      return [];
    }

    if (tokenOptions.tokenType && tokenOptions.tokenType !== 'NATIVE') {
      console.warn(`⚠️  [Bitcoin Cash] Token type ${tokenOptions.tokenType} not supported, skipping.`);
      return [];
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`⚠️  [Bitcoin Cash] Failed to resolve wallet address: ${message}`);
    }
  ...
  getRecipientAddress(user: User): string | null {
    return (user as User & { bitcoinCashAddress?: string | null }).bitcoinCashAddress ?? null;
  }
  ...
  async estimateTransfer(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unknown Bitcoin Cash estimation error';
  ...
  async sendTransfer(
    ...
    if (amountToken <= 0) {
      throw new Error('[Bitcoin Cash] Transfer amount must be greater than zero');
    }

    const config = readBitcoinCashConfig();
    if (!config.wif) {
      throw new Error('[Bitcoin Cash] BCH_WIF is required for walletless transfers');
    }

    const client = await this.getClient();
    const senderAddress = this.ensureStaticWalletAddress(config);
    if (!senderAddress) {
      throw new Error('[Bitcoin Cash] Unable to determine sender address');
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`[Bitcoin Cash] Failed to scan UTXOs: ${message}. Ensure scantxoutset is supported by the RPC node.`);
    }

    if (unspents.length === 0) {
      throw new Error(`[Bitcoin Cash] No UTXOs found for the sender address ${senderAddress}`);
    }
    ...
    if (totalInputSat < amountSat) {
      throw new Error(`[Bitcoin Cash] Insufficient funds: have ${totalInputSat / 1e8} BCH, need ${amountToken} BCH`);
    }
    ...
    if (totalInputSat < amountSat + finalFee) {
      throw new Error(`[Bitcoin Cash] Insufficient funds to cover fees: have ${totalInputSat / 1e8} BCH, need ${(amountSat + finalFee) / 1e8} BCH`);
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`[Bitcoin Cash] Failed to broadcast transaction: ${message}`);
    }
  }
...
export const bitcoinCashGasTokenNetworkAdapter = new BitcoinCashGasTokenNetworkAdapter();
