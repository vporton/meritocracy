...
- Raw KYC documents, DOB, address, photo, document/ID number, biometrics, provider reports, and sensitive identity attributes never enter staging, target data, canonical export, CI artifacts, or operator backups. The only target KYC disposition is the G2-approved minimum attestation; source fields excluded by that policy require explicit redacted migration exceptions. A formal Foundation country-policy resolution is required before any country is enabled for KYC or payouts. Certified public projections may expose only their allowlisted principal, payout address, payment/evaluation history, and, in ban-voting only, provider/public handle-or-ORCID-ID plus verification state. Published treasury receiving configuration is separate.
