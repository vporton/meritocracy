...
14. **IMPLEMENTED 2026-08-08 (single-toolchain adoption):** The project owner selected the exact ZenDB Motoko `1.4.1` compiler for the repository, superseding items 10–12's earlier `0.16.3` rejection decision. `mops.toml` now pins `moc = "1.4.1"`; the two legacy `motoko-base` import aliases resolve to the exact upstream `moc-1.4.1` commit `d1b336e142bec76577e3d63d9f1fac74de48df0a`; the source-identical `identify@0.0.2` backend-only package view excludes only its upstream frontend E2E actor, which Motoko 1.4 rejects in a library search path; and the management-canister Candid declaration is vendored and hashed for explicit `--actor-idl` resolution. `mops test` (4 files), `mops check`, `mops build`, `mops check-stable`, and both exact-source embedded ZenDB compiler probes pass with Motoko 1.4.1. This is a compiler/dependency compatibility change only: ZenDB is still not imported by a target canister, no storage method or data is enabled, and the required bounded persistence/recovery/RBAC/upgrade/low-cycle proofs remain M1/G2 blockers.
